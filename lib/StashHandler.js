...
var StashHandler = function(options) {
  ...
  server.post(handler_options.path, function(req, res, next){
...
StashHandler.prototype.getStashApi = function(project) {
  var stash = new Stash({
    url: this.options[project.provider.slug].url,
    auth: {
      type: 'oauth1',
      token: project.service_auth.token,
      tokenSecret: project.service_auth.tokenSecret,
      consumerKey: this.options[project.provider.slug].consumerKey,
      consumerSecret: this.options[project.provider.slug].consumerSecret
    }
  });
...
StashHandler.prototype.pushHandler = function(event, cb) {
  ...
  var request = {
    ...
    owner: event.payload.repository.project.key,
  ...
  // parse out host
  try {
    var link = event.payload.links[0].self[0].href;
