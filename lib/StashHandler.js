...
StashHandler.prototype.validateOptions = function() {
  ...
  for(var slug in providers){
    ...
    for(var r in required){
      if(!provider[required[r]]){
        throw new Error("Missing required stash config: " + required[r])
      }
    }
  }
}
...
StashHandler.prototype.getStashApi = function(project) {
  var stash = new Stash({
    url: this.options.providers[project.provider.slug].url,
    auth: {
      type: 'oauth1',
      token: project.service_auth.token,
      tokenSecret: project.service_auth.tokenSecret,
      consumerKey: this.options.providers[project.provider.slug].consumerKey,
      consumerSecret: this.options.providers[project.provider.slug].consumerSecret
    }
  });
  ...
  this.api.findProjectByRepo(request, function(error, project){
    ...
    self.fetchProboYamlConfigFromStash(project, request.sha, function(error, config) {
      ...
      if(error){
        ...
        return self.log.info({config: config}, "Probo Yaml Config file");
      }
