...
BitbucketHandler.prototype.pushHandler = function(event, cb) {
  ...
  var request = {
    ...
    service: null,      // will always be bitbucket
    ...
    branch: event.payload.refChanges.refId.split('/')[2], // refId: refs/heads/feature
    sha: event.payload.refChanges[0].toHash,
    payload: event.payload,
  }
  request.slug = util.format("%s/%s", request.owner, request.repo);

  // parse out host, not doing this for bitbucket
  /*try {
    ...
    // request.service = util.format("bitbucket:%s", request.host)
  } catch(e){
    self.log.error({err: e, event: event},
                   "Failed to find host for Bitbucket push, ignoring push");
    return;
  }*/
