...
Stash.prototype.http = function(opts, cb) {
  ...
  opts = this.utils.defaults(opts, {
    uri: this.url.replace(/\/$/, '') + '/' + opts.path.replace(/^\//, ''),
    oauth: this.oauth,
    json: true,
  });
...
Stash.prototype.statuses = {
  create: function(status, opts, cb) {
    ...
    if (validStates.indexOf(status.state) < 0) {
      return cb(new Error('statuses.create status.' + status.state + ' required to be one of ' + validStates.join(', ')));
    }
...
Stash.prototype.pullRequests = {
  /**
   * Gets pull requests for a project matching params
   * @param {object} opts - Options hash
   * @param {string} opts.projectKey - Project Key (ex: TEST)
   * @param {string} opts.repositorySlug - Repository slug (ex: test-repo)
   * @param {string} opts.branch - Branch spec (refs/heads/feature)
   * @param {string} opts.state - (optional, defaults to OPEN). Supply ALL to return pull request in any state. If a state is supplied only pull requests in the specified state will be returned. Either OPEN, DECLINED or MERGED.
   */
  getAll: function(opts, cb) {
    if (typeof opts == 'function') {
      cb = opts;
    ...
    var path = util.format('/rest/api/1.0/projects/%s/repos/%s/pull-requests',
                           opts.projectKey, opts.repositorySlug);

    var qs = {
      at: opts.branch,
      // Stash default is 'incoming', but if a branch is specified,
      // we want to know the PR for this branch, not where it's being merged into
      direction: opts.branch ? 'outgoing' : void 0,
      state: opts.state,
    };
    delete opts.branch;
    delete opts.state;
    ...
    this.http(params, function(err, res, data) {
      if (err) return cb(err);
      ...
      // drill down into actual PR array
      var pullRequests = data.values;
